pipeline{
            ...
            steps{
                script{
                    withSonarQubeEnv("Sonarserver") {
                        ...
                        sh "${tool("sonar")}/sonarqube-scanner/bin/sonar-scanner"
                    }
	...
	// 	always {
    //         "Success"
	// 		//mail bcc: '', body: "<br>Project: ${env.JOB_NAME} <br>Build Number: ${env.BUILD_NUMBER} <br> URL de build: ${env.BUILD_URL}", cc: '', charset: 'UTF-8', from: '', mimeType: 'text/html', replyTo: '', subject: "${currentBuild.result} CI: Project name -> ${env.JOB_NAME}", to: "prafullb1997p@gmail.com";  
	// 	 }
	//    }
}
